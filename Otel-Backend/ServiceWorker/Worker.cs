...
namespace ServiceWorker
{
	public class Worker : BackgroundService
    {
        ...
        {
            await _bus.ReceiveAsync<LoanRequest>(QueueType.Processing, (message, args) =>
            {
                Task.Run(() => { ProcessMessage(message, args); }, cancellationToken);
            });
        }

        private async Task ProcessMessage(LoanRequest message, BasicDeliverEventArgs args)
        {
            ...
            using (var activity = Activity.StartActivity("Process Message", ActivityKind.Consumer, parentContext.ActivityContext))
            {
                ...
                //foreach(var productId in message?.Loan?.ProductId)
                //{
                    tasks.Add(Task.Run(() => _catalogService.GetProduct(message.Loan.ProductId)));
                    tasks.Add(Task.Run(() => _catalogService.GetProduct(message.Loan.ProductId)));
                //}
